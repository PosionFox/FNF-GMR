...
if (input.up) {if (curSelected > 0) curSelected--; audio_play_sound(scrollMenu, 0, false);}
if (input.down) {if (curSelected < optionAmount[category]) curSelected++; audio_play_sound(scrollMenu, 0, false);}
...
if (curSelected = 0) {
	if (input.left) {if (category > 0) category--; audio_play_sound(scrollMenu, 0, false);}
	if (input.right) {if (category < 2) category++; audio_play_sound(scrollMenu, 0, false);}
}
