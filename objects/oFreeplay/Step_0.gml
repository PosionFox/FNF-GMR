...
if (input.enter)
{
	...
	var o = instance_create_depth(x, 0, -10000, oFade);
	o.roomTo = MainGame;
}
...
if (lastSelected != curSelected)
{
	...
	songData = loadSongData("data/" + string(songsList[curSelected]) + "/" + string(songsList[curSelected]) + ".json");
	
	// play the songs music
	audio_stop_sound(musicPlaying);
	musicPlaying = audio_play_sound(asset_get_index(songData.instFile), 10, true);
}
