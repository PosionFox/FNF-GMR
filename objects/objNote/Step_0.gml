...
var trueNoteSpeed = (curNoteSpeed) * global.deltaMultiplier;

y -= trueNoteSpeed;
if (yOff != 0) {y -= yOff; yOff = 0;}
if (switchTurn) sprite_index = sprDebugNotes;
...
if (!global.playVoice) {
	with (instance_place(x, y - 120, objArrowBtnParent))
		if (enemy)
			global.playVoice = 1;
}
/*
...
if (beingHeld)
{
	...
	global.hp += 0.08;
	with (objArrowBtnParent) if (dir = other.image_index) && (image_index > 4) image_index = 0;
	objBoyfriend.holdAnimation = true;
	
	if (keyboard_check_released(keyPressedWith)) || (keyboard_check_released(altkeyPressedWith))
	{
		beingHeld = false;
		curNoteSpeed = global.noteSpeed;
		objBoyfriend.holdAnimation = false;
	}
...
if (enemyHeld)
{
	...
	curNoteSpeed = 0;
	sliderLength -= global.noteSpeed * global.deltaMultiplier;
	objEnemy.holdAnimation = true;
	
	if (sliderLength <= 0)
	{
		instance_destroy();	
		objEnemy.holdAnimation = false;
	}
}
